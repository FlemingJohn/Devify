...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView }) => {
  const NavItem = ({ icon: Icon, label, view }: { icon: any, label: string, view: ViewType }) => (
    <button
      ...
      className={`flex md:flex-row flex-col items-center gap-1 md:gap-4 md:px-6 py-2 transition-colors duration-200 w-full md:text-left text-center
        ${currentView === view ? 'text-white' : 'text-[#b3b3b3] hover:text-white'}`}
    >
      <Icon size={24} className={currentView === view ? 'text-white' : 'text-[#b3b3b3]'} />
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex w-64 bg-black h-full flex-col pt-6 shrink-0 z-[100]">
        ...
        <div className="border-t border-[#282828] mx-6 py-4 overflow-y-auto flex-1">
          ...
          <ul className="space-y-3 text-sm text-[#b3b3b3] font-medium">
            <li className="hover:text-white cursor-pointer truncate">React Masterpieces</li>
            <li className="hover:text-white cursor-pointer truncate">Backend Grooves</li>
            <li className="hover:text-white cursor-pointer truncate">Open Source Hits</li>
            <li className="hover:text-white cursor-pointer truncate">Design Systems Vol.1</li>
          </ul>
        </div>
      </div>

      {/* Mobile Bottom Nav */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-black/95 backdrop-blur-lg border-t border-white/5 flex items-center justify-around z-[100] px-4">
        <NavItem icon={Home} label="Home" view={ViewType.HOME} />
        <NavItem icon={Search} label="Search" view={ViewType.SEARCH} />
        <NavItem icon={Library} label="Library" view={ViewType.LIBRARY} />
        <button
          onClick={() => setView(ViewType.WRAPPED)}
          className={`flex flex-col items-center gap-1 transition-colors duration-200 text-center
            ${currentView === ViewType.WRAPPED ? 'text-white' : 'text-[#b3b3b3] hover:text-white'}`}
        >
          <div className="bg-gradient-to-tr from-[#ff00d4] to-[#00f2ff] p-1 rounded shadow-lg">
            <Sparkles size={16} className="text-white" />
          </div>
          <span className="font-bold text-[10px]">Wrapped</span>
        </button>
      </div>
    </>
  );
